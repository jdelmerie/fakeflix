...
export class MovieComponent implements OnInit, OnDestroy {
  ...
  fav(listName: string = 'favorites') {
    this.action(
      this.api.getFavs(1),
      listName,
      this.api.fav(this.movieId),
      this.alertSuccess(this.alertAdded + listName)
    );
  }
  ...
  watchlist(listName: string = 'watchlist') {
    this.action(
      this.api.getWL(1),
      listName,
      this.api.wl(this.movieId),
      this.alertSuccess(this.alertAdded + listName)
    );
  }

  private action(
    apiLinkGetData: Observable<any>,
    listName: string,
    apiLink: Observable<any>,
    alert: any
  ) {
  ...
  deleteWL(movieId: number, listName: string = 'watchlist') {
    this.delete(
      listName,
      movieId,
      this.api.unwl(movieId),
      (this.disabledWL = false)
    );
  }
  ...
  private delete(
    ...
    if (this.api.isSessionIdExisting()) {
      ...
      this.confirmSuppression(
        `Do you really want to delete this movie from your ${listName} ?`
      ).then((res) => {
        if (res.isConfirmed) {
          apiLink.subscribe({
  ...
  private checkFavOrWL(datas: any, movieId: number) {
    return datas.filter((item: any) => item.id == movieId).length === 1
      ? true
      : false;
  }
